...
const createAnswer = async function (req, res) {
    try {
        ...
        const { questionId, answeredBy, text } = requestBody;
...
const getAllAnswers = async (req, res) => {
    try {
        ...
        //Validating questionId.
        if (!validator.isValidObjectId(questionId)) {
            return res.status(400).send({ status: false, message: `${questionId} is not a valid question id in URL params.` });
        }

        const searchQuestion = await questionModel.findOne({ _id: questionId });
        if (!searchQuestion) {
            return res.status(404).send({ status: false, message: `Question doesn't exists by ${questionId} or has been deleted.` });
        }
        ...
        if (Array.isArray(fetchAnswers) && fetchAnswers.length === 0) {
            return res.status(404).send({ status: false, message: `No answers found for ${questionId}.` });
        }

        return res.status(200).send({ status: true, message: `Answer fetched successfully for ${questionId}.`, data: fetchAnswers });
...
const updateAnswer = async (req, res) => {
    try {
        ...
        if (!findAnswer) {
            return res.status(400).send({ status: false, message: `No answer found by ${answerId}` });
        }
        ...
        if (!validator.validString(text)) {
            return res.status(400).send({ status: false, message: "Please provide the answer text to update." });
        }
